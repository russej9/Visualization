//group members: Emma Bradford, Ben Swanson
  ...
  .then(data => {  //ARROW function - pass data as a parameter into this anonymous function
    ...
    data.forEach(d => { //ARROW function - for each object in the array, pass it as a parameter to this function
		...
		let tokens = d.start.split("-");
  		...
  		let cx = mapFromDomainToRange(0, 365, timelineX1, timelineX2, computeDays(d.start)); //******* don't use 0, use mapFromDomainToRange. Hint- use 365 days and don't worry about leap years... it is ok
  		let cy = getYearPosition(d.year, gapBetweenTimelines, startOfTimelinesY); //******* don't use 0, use getYearPosition
  		let radius = mapFromDomainToRange(1, 160, 10, 200, d.cost); //******* don't use 10, use mapFromDomainToRange.  The max cost is 160, and the min cost is 1
